...
v5.0 Deprecated opt_P option for PTM
	--uses the searchTheseDBs file to search for tags that use protein coordinates instead of genomic, e.g. PTM and mutAligner
	--these DBs are launched with gene annot
=cut
my $user=`whoami`;chomp $user;$user||="www-data";my $qsub=`which qsub`;chomp $qsub;my $qstat=`which qstat`;chomp $qstat;
...
use vars qw($opt_j $opt_C $opt_W $opt_I $opt_E $opt_R $opt_A $opt_G $opt_w $opt_z $opt_F $opt_Q $opt_x $opt_m $opt_H $opt_f $opt_a $opt_o $opt_i $opt_d $opt_T  $opt_h $opt_t $opt_c $opt_g $opt_n $opt_l $opt_b $opt_B $opt_k $opt_r $opt_s $opt_N $opt_v $opt_S $opt_p $opt_P $opt_V);
 getopts("f:o:c:d:i:T:t:l:B:k:p:m:F:A:j:P:rvRzsbcaehxgnHEINSwQWIGCV");
 umask(0000);
my $usage=qq{
	$0 -i <input filename>
	...
	[CONDITIONAL]
		...
		-P <if specified, runs protein modules (takes a list of protein DBs)>
...
if (!$opt_g){
	if ($qsub=='' && -e '/usr/local/bin/qsub'){
		$qsub='/usr/local/bin/qsub';
		if (-e $qsub){
			$qstat="/usr/local/bin/qstat";
		}
	}elsif ($qsub == ''){
		$qsub=`locate qsub | grep qsub\$ | head -n1`;chomp $qsub;
		if ($qsub){
			$qstat=`dirname $qsub`;chomp $qstat;
			$qstat.="/qstat";
		}
	}
}
...
my $exceptions='(\_af$|PTM$|.*funseq.txt|.*uniprot.txt|.*FlankingSequence|.*funseq2.txt|mutAligner)';##.*Encode.*.txt|
...
our $bin=`dirname $0`;chomp $bin;
my $annovar="perl $bin/annotate_variation_ABCC2IP.pl ";
if ($opt_V){
...
if (!$opt_P) {##if not specified on command line, check if it is in the database file
	my @protDBs= join ("",@files)=~/$opt_o\_(dbPTM|mutAligner|PTM)/g;
	$opt_P.= join (",",@protDBs);
	# print "opt_P was not set and protDBs built...($opt_P)\n";
}
if ($opt_P){#specified on command line, but must make sure it's incorporated into results
	foreach my $protDB (split(",",$opt_P)){
		push (@files,"$opt_o\_$protDB\n") and print "Adding $opt_o\_$protDB\n"  if (join ("",@files)!~/$protDB/);
	}
}
# print join ("\n",@files);exit;
print "\n$opt_P (ProteinDB modules) is set?\n";
if ($opt_z){
	push (@files,"$opt_o\_FlankingSequence") if (join("",@files)!~/FlankingSequence/);
}
my $siftver=(join("",@files)=~/($opt_o\_ljb\d+\_sift)/)?$1:"$opt_o\_siftv63";
# die $siftver . "\n" .  join ("",@files);exit;
...
#			system ("rm $opt_i\\_* -f\n");
	...
	for (my $k=0;$k<$ct;$k++){
		...
		if ($opt_P){$ensembl.=" --protDB $opt_P";}
		$annot="$annovar --buildver $opt_o $opt_Q --geneanno $ensembl --keepline --relpos  $opt_i\_$k $opt_d";
		if ($opt_v){
...
#}
my %db_size_hash;my $af;my %otherArgs;
	...
	for (my $k=0;$k<$ct;$k++){#make pbs bat files for all of the databases
		...
		if ($#files>-1 && !$opt_n){#runs if there are files and opt_n is not specified
			for (my $i=0;$i<=$#files;$i++){
				my $other;
				if ($files[$i]=~/^(\S+)\t(.*)/){
					$files[$i]=$1;$otherArgs{$1}=$2;
				}
				if (exists $otherArgs{$files[$i]}){
					$other=$otherArgs{$files[$i]};
				}
				$files[$i]=~s/[\r\n]*$//;my ($db_dir,$db_file);
				...
				if( ! -e "$db_dir/$db_file.idx" && $db_file!~/vcf/){
				...
				if ( -e "$db_dir/$db_file.idx"){#indexing available so you do not need a lot of space
					$int="16";$idxd=1;
					my $batchsize=($db_file=~/(wgencode|dfam|rptmask|avsnp|regulome|hrcr)/i)?'1000':'100k';
				...
				print "[Checkpoint#" .__LINE__."]PBS$k\_$i...($size)$int($toDo)\n";
				if ($db_file=~/vcf/){
					$toDo="$annovar --buildver $opt_o $opt_c --dbtype vcf --filter $other --keepline --vcfdbfile $db_file $opt_i\_$k $db_dir";
					$int=8;
				}elsif (($db_file=~/(avsnp14\d+|1000g|clinvar|snp1[243]\d|regulome\d+|avsift|ljb|vcf|cg69|siftv63|hapmap|nci60|rbpvar_v|provean|esp6500|kaviar|hrcr|ExAC|(ALL|AFR|AMR|EAS|EUR|SAS).sites)/i)){#large files with index
					$toDo="$annovar --buildver $opt_o $opt_c --dbtype $db_file --filter $other --keepline $addon $opt_i\_$k $db_dir";
					$int=8 if ($db_file!~/avsnp\d+/);#we do this because it is filter option and memory should not be an issue
					...
					## function to find whether or not the custom database is filter or region annotation
					if (-e "$db_dir/config.ini"){
						my @var=split("\n",`grep -A1 -B1 $db_file $db_dir/config.ini`);
						my $customId='';
						if ($var[1]=~/userdefined_annotdb(\d+)/){
							$customId=$1;
							if ($var[2]=~/alleles_userdefined_db$customId=on/){
								print "Found $var[2]\n\n";
								$toDo="$annovar --buildver $opt_o $opt_c --dbtype $db_file --filter --force --keepline $addon $opt_i\_$k  $db_dir";
								$int=8;
							}#else keep default
						}#else keep default
					}# else keep default
...
if (defined $opt_C){
	...
	CHUNK: foreach (my $k=0;$k<$ct;$k++){
		...
		system ("perl /code/annovar/parseData/convert4Uniprot.pl -a $opt_i\_$k.variant_function.collapsed -M $opt_i\_$k.$opt_o\_ProtPos\n");
	...
	CHUNK: foreach (my $k=0;$k<$ct;$k++){
		...
		FILE: for (my $i=0;$i<=$#files;$i++){
			...
			chomp $files[$i];
			($files[$i])=split("\t",$files[$i]);
			# print "------------------>finding dirname for $files[$i]\n";
			my ($dirname,$filename)=(`dirname $files[$i]`,`basename $files[$i]`);chomp $dirname;chomp $filename;
			$filename=~s/\r//g;
			next if (!$filename);
		...
		system ("perl /code/annovar/parseData/convert4Uniprot.pl -a $opt_i\_$k.variant_function.collapsed -M $opt_i\_$k.$opt_o\_ProtPos -h\n");
...
#cleanup($ct);
	...
	if (keys %err==0 && -e "$opt_i.$opt_o\_comments" && !-e "filtered_results.txt"){
	...
	if (-e "config.ini" && -e "$convertible" && !-z "$convertible"){
		$addon="$convertible ". $addon;
	}
	if ($addon){
		system STDERR "about to paste the comments\n";
		my $dbcount=`wc -l $opt_f`;chomp $dbcount;
		$dbcount+=10;#ProtPOs,Annot,Gene,RelPos, Gene Ori, Chr,Start,Stop, Allele1,Allele2
		print STDERR "cut -f 1-$dbcount $out.output > $out.output1;paste $out.output1 $addon >$out.output;\n";
...
sub monitor{
	my $targets=shift;
	print "\n-----------------------------------\n[INFO] In monitoring grid jobs: $targets\n\n\n";
...
sub printPBS{
	...
	print OUT "echo 'System Resource Info: ' `uname -a`\n";
	print OUT  "cd $cwd\n";
	print OUT  "$cmd \n";
	close OUT ;
	...
	if (!$opt_g){
		...
		# print "[INFO] Running locally....$opt_i.$name.bat\n";exit;
		eval{
			system ("chmod +x PBS.$opt_i.$name.bat;./PBS.$opt_i.$name.bat\n");
		};
		if ($?){
...
sub checkPBSAccess{
	...
	if ($?){
		...
		$qsub='';$qstat;
		#try to see if /usr/local/bin/ works
		if (-e "/usr/local/bin/qsub"){
			...
			#try
			$qsub=`locate qsub | grep qsub\$ |head -n1`;chomp $qsub;
			$qstat=`dirname $qsub`;chomp $qstat;$qstat.="/qstat";
		}
		if (!$qsub){
			print STDERR "[WARN] You cannot use the grid to run your jobs (system err)$?\n";$opt_g=1;
		}else{
			print STDERR "[INFO] Submission to grid is allowed using $qstat\n";
		}
...
sub runNMD{
	...
	if ($opt_I || ($opt_f && `grep sift $opt_f|wc -l`>=1 )){
			print STDERR "Running $bin/find_and_predict_NMD_indels.pl -I $NMDfile -o $NMDfile.PredIndel -v $siftver  \n";
			## use this perl module because of the SQLite.pm and Dynaloader.pm installed here
			system ("/opt/nasapps/applibs/perl-5.16.2/bin/perl $bin/find_and_predict_NMD_indels.pl -I $NMDfile -o $NMDfile.PredIndel -v $siftver \n");
...
sub runExceptions{
	...
	if ($runcmd=~/funseq/i && $opt_C && $opt_W){#deprecated
		print "Funseq is fun!\n";
		$runcmd=~s/-V/-F -N/;
		$runcmd=~s/smart_wrpr.pl/smart_wrprIP.pl/g;
		$runcmd.= " -o $block";
	}elsif ($runcmd=~/funseq2/i){
		print "changing it up! funseq\n";
		$runcmd.= " -j $block";
	}
	print "RUNNING:$runcmd\n";
	# printPBS ("$runcmd\n");
	printPBS("$exception\_$block",'8',$runcmd);
	return;
}
